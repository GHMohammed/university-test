...
type RowStatus = 'valid' | 'missing_fields' | 'invalid_email' | 'duplicate_email' | 'duplicate_instructor_code' | 'duplicate_in_file';
...
const REQUIRED_COLUMNS = ['full_name', 'email', 'password', 'phone', 'instructor_code', 'department'];
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export function BulkInstructorImport() {
  const { t } = useI18n();
  ...
  const downloadTemplate = () => {
    const ws = XLSX.utils.aoa_to_sheet([
      ['full_name', 'email', 'password', 'phone', 'instructor_code', 'department'],
      ['Dr. Sara Khan', 'sara@example.com', 'Pass1234', '0501234567', 'INS001', 'CS'],
    ]);
  ...
  const exportCurrentData = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('full_name, email, phone, instructor_code, department')
        .in('id', ids)
        .order('full_name');
      ...
      const ws = XLSX.utils.json_to_sheet(rowsOut, {
        header: ['full_name', 'email', 'phone', 'instructor_code', 'department'],
      });
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Instructors');
      const stamp = new Date().toISOString().slice(0, 10);
      XLSX.writeFile(wb, `instructors_export_${stamp}.xlsx`);
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data: existingProfiles } = await supabase.from('profiles').select('email, instructor_code');
      const existingEmails = new Set((existingProfiles || []).map(p => p.email.toLowerCase()));
      const existingCodes = new Set((existingProfiles || []).map(p => p.instructor_code?.toLowerCase()).filter(Boolean));
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        const row: ParsedRow = {
          ...
          instructor_code: String(raw.instructor_code || raw.Instructor_Code || raw.INSTRUCTOR_CODE || '').trim(),
        ...
        if (row.instructor_code && codesInFile.has(row.instructor_code.toLowerCase())) {
          row.status = 'duplicate_in_file';
          row.statusDetail = `instructor_code (row ${codesInFile.get(row.instructor_code.toLowerCase())})`;
          parsed.push(row);
          continue;
        }
        ...
        if (existingEmails.has(row.email)) {
          ...
          if (row.instructor_code) codesInFile.set(row.instructor_code.toLowerCase(), row.rowNum);
          continue;
        }

        if (row.instructor_code && existingCodes.has(row.instructor_code.toLowerCase())) {
          row.status = 'duplicate_instructor_code';
          parsed.push(row);
          emailsInFile.set(row.email, row.rowNum);
          if (row.instructor_code) codesInFile.set(row.instructor_code.toLowerCase(), row.rowNum);
          continue;
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      try {
        await createMutation.mutateAsync({
          ...
          instructor_code: row.instructor_code || undefined,
  ...
  const statusBadge = (row: ParsedRow) => {
    const map: Record<RowStatus, { variant: 'default' | 'destructive' | 'secondary' | 'outline'; label: string }> = {
      ...
      duplicate_instructor_code: { variant: 'secondary', label: t('bulk_students.status_dup_code') },
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex flex-col items-center gap-4 py-8">
            ...
            <h3 className="text-lg font-semibold">{t('bulk_instructors.title')}</h3>
            <p className="text-sm text-muted-foreground">{t('bulk_instructors.subtitle')}</p>
            <div className="flex gap-3">
              ...
              <Button onClick={() => fileInputRef.current?.click()}>
                <Upload className="h-4 w-4 me-2" />{t('bulk_instructors.upload')}
              </Button>
            </div>
            <input
              ref={fileInputRef}
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold">{t('bulk_instructors.preview_title')}</h3>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('crud.instructor_code')}</TableHead>
              ...
              <TableBody>
                {rows.map((row, idx) => (
                  <TableRow key={idx} className={row.status !== 'valid' ? 'bg-destructive/5' : ''}>
                    ...
                    <TableCell>{row.instructor_code}</TableCell>
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex flex-col items-center gap-4 py-8">
            ...
            <h3 className="text-lg font-semibold">{t('bulk_instructors.importing')}</h3>
