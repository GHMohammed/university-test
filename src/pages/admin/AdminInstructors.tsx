...
import { BulkInstructorImport } from '@/components/instructors/BulkInstructorImport';

type InstructorRow = { id: string; full_name: string; email: string; phone: string | null; instructor_code: string | null; department: string | null; status: string; created_at: string };

export default function AdminInstructors() {
  const { t } = useI18n();
  const { data = [], isLoading } = useProfilesByRole('instructor');
  ...
  const [form, setForm] = useState({ email: '', password: '', full_name: '', phone: '', instructor_code: '', department: '' });

  const openCreate = () => { setEditing(null); setForm({ email: '', password: '', full_name: '', phone: '', instructor_code: '', department: '' }); setDialogOpen(true); };
  const openEdit = (r: InstructorRow) => { setEditing(r); setForm({ email: r.email, password: '', full_name: r.full_name, phone: r.phone || '', instructor_code: r.instructor_code || '', department: r.department || '' }); setDialogOpen(true); };

  const handleSubmit = async () => {
    try {
      if (editing) {
        await updateMutation.mutateAsync({ id: editing.id, full_name: form.full_name, phone: form.phone || null, instructor_code: form.instructor_code || null, department: form.department || null });
        toast.success(t('save'));
      } else {
        await createMutation.mutateAsync({ ...form, role: 'instructor' });
  ...
  const columns: Column<InstructorRow>[] = [
    ...
    { key: 'instructor_code', header: t('crud.instructor_code'), sortable: true },
  ...
  return (
    <DashboardLayout>
      <PageHeader title={t('admin.instructors')} actions={
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setShowBulkImport(!showBulkImport)}>
            <Upload className="h-4 w-4 me-2" />{t('bulk_instructors.title')}
          </Button>
          <Button onClick={openCreate}><Plus className="h-4 w-4 me-2" />{t('crud.add_instructor')}</Button>
        </div>
      } />
      {showBulkImport && (
        <div className="mb-6">
          <BulkInstructorImport />
        </div>
      )}
      <DataTable columns={columns} data={data as InstructorRow[]} isLoading={isLoading} searchKeys={['full_name', 'email', 'instructor_code']} />

      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>{editing ? t('crud.edit_instructor') : t('crud.add_instructor')}</DialogTitle></DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid gap-2"><Label>{t('crud.instructor_code')}</Label><Input value={form.instructor_code} onChange={e => setForm({ ...form, instructor_code: e.target.value })} /></div>
            </div>
